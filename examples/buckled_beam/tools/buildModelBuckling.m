function [M, C, K, fnl, fext, outdof, PlotFieldonDefMesh] = buildModelBuckling(nElements,l,h,b,E)
% build model of a pinned-pinned von Karman beam
...
rho     = 7850;   % density
nu      = 0.3;    % nu
kappa   = 7e6;    % material damping modulus
...
MyMesh.set_essential_boundary_condition([1],[1 2],0) % Pinned-Pinned
MyMesh.set_essential_boundary_condition([nNodes],[2],0) % Pinned-Pinned
...
outnode = ceil(MyMesh.nNodes);
outdof = outnode*3-2; % axial direction
