function [Ktilde, f1tilde , f2tilde] = functFromTensorsSFP(M, C, K, fnl, fExt, unstable_fp, varargin)
...
for ii = 1:n
    for i = 1:n
        for j = 1:n
            temp = diff(diff(qdot(ii), q(i)),q(j));
            f1tilde(ii, i,j) = subs(temp, q , unstable_fp);
        end
    end
end

f2tilde = zeros(n,n,n,n);
for jj = 1:n
for ii = 1:n
    for i = 1:n
        for j = 1:n
            temp = diff(diff(diff(qdot(jj), q(ii)),q(i)), q(j));
            f2tilde(jj, ii, i,j) = subs(temp, q , unstable_fp);
        end
    end
end
end
